...
* **وضعیت**: فعال
* **تاریخ**: 2026-06-04 (به‌روزرسانی: 2026-09-06 — ابزارِ `-Pliquibase-ops`/`just db-*` + قراردادِ `DB_URL`)
* **نویسندگان**: تیم معماری Nova
* **دسته‌بندی**: پایگاه‌داده / استقرار (Database / Deployment)
* **تیکت‌های مرتبط**: [LN-59738](https://jira.dotin.ir/browse/LN-59738)

> این Runbook نحوهٔ اجرای امن مهاجرت‌های schema را در یک ناوگان چندنمونه‌ای مستند می‌کند. مدل پیشین، مهاجرت
> in-app روی هر boot بود؛ در مدل فعلی، اپلیکیشن migration را اجرا نمی‌کند و یک runner یگانه باید پیش از rollout
> آن را انجام دهد.
...
زمان اجرا از `spring.liquibase` در `nova-config » kv/expenditures/loan/nova/application/datasource.yml` پیکربندی می‌شود
(مسیرِ master بدونِ تغییر: `classpath:db/changelog/db.changelog-master.xml`).

## وضعیت پیشین در برابر وضعیت فعلی

### وضعیت پیشین — مهاجرتِ in-app روی هر boot

در تنظیمات پیشینِ `nova-config » kv/expenditures/loan/nova/application/datasource.yml`:
...
با `enabled: true`، **هر نمونهٔ Nova روی هر boot مهاجرت‌ها را اجرا می‌کرد**. در یک ناوگان N نمونه‌ای یعنی N
...
### وضعیت فعلی — runner اختصاصی، اپ‌ها هرگز migrate نمی‌کنند

در config فعلی، `spring.liquibase.enabled` در `nova-config` روی **`false`** است تا هیچ نمونهٔ اپلیکیشنی schema را
تغییر ندهد. مهاجرت‌ها باید با یک **runner اختصاصی** و **پیش از** rolling کردن podهای اپ اجرا شوند:

<div dir="ltr">

```yaml
# nova-config » kv/expenditures/loan/nova/application/datasource.yml
spring:
  liquibase:
    change-log: classpath:db/changelog/db.changelog-master.xml
...
### مسیر canonical — profile `-Pliquibase-ops` + recipeهای `just db-*`
...
`changeLogFile=db/changelog/db.changelog-master.xml` از classpath resolve می‌شود — همان master پیکربندی‌شده برای
runtime اپ؛ پس **`just install` (reactor → `.m2`) باید اول اجرا شود** تا jarهای changelogِ
pangaea/expression-kit در دسترسِ پلاگین باشند. اتصالِ DB از `DB_URL`، `DB_USERNAME` و `DB_PASSWORD` یا overrideهای
`-Dliquibase.*` می‌آید؛ هیچ secret هاردکد نمی‌شود.

recipeهای `just db-*` در `justfile` ریشهٔ Nova، فایل `container/.env` را در صورت وجود source می‌کنند و حضورِ
`DB_URL`، `DB_USERNAME` و `DB_PASSWORD` را guard می‌کنند:

<div dir="ltr">

```bash
just db-status                 # liquibase:status   — changesetهای apply‌نشده
just db-validate               # liquibase:validate — اعتبارسنجی بدونِ نوشتن روی DB
just db-update                 # liquibase:update   — اعمال changesetهای pending
just db-sql                    # liquibase:updateSQL — dry-run، SQL را در container/target/ می‌نویسد
just db-tag <release>          # liquibase:tag      — tag وضعیت فعلی DB
just db-rollback <release>     # liquibase:rollback — rollback تا یک tag
just db-rollback-count 2       # liquibase:rollback — rollback آخرین N changeset
just db-history                # liquibase:history  — تاریخچهٔ استقرار
just db-release-locks          # liquibase:releaseLocks — آزادسازیِ قفلِ گیرکرده

# شکلِ خام (بدونِ justfile؛ متغیرهای فایل برای Maven export می‌شوند):
set -a
...
- **tag-per-release.** پیش از هر استقرار `just db-tag <release>` (یا `liquibase tag <release>`) بزنید تا
  `just db-rollback <release>` همیشه یک هدفِ مشخص داشته باشد.
...
- پیکربندی Liquibase در زمان اجرا: `nova-config » kv/expenditures/loan/nova/application/datasource.yml`
  (`spring.liquibase`).
...
- profileِ `liquibase-ops` (maven-plugin): `nova » container/pom.xml`؛ recipeهای operator: `nova » justfile`
  (`db-*`).
