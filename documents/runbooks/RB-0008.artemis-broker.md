...
journal و paging روی همان دیسکی بودند که Docker image store و بقیهٔ کانتینرها از آن استفاده می‌کنند. وقتی مصرف از
`max-disk-usage=90` گذشت، Artemis همهٔ producerها را با `AMQ212054` بلاک کرد و این علتِ اصلی قطعی بود. اندازهٔ صف‌ها
اصلاً موضوع نبود؛ همسایه‌ها دیسک را پر کردند.

`docker-compose.yml` حالا هر شش volume داده را با `ARTEMIS_DATA_ROOT` پارامتری می‌کند (پیش‌فرض `/disk-c/artemis`).
یک mount اختصاصی به آن بدهید — یعنی فایل‌سیستمی که Docker و سرویس‌های دیگر روی آن نمی‌نویسند.

> **هشدار مهاجرت:** این bind mountها جایگزین named volumeهای قبلی می‌شوند. اگر بدون کپی داده استک را بالا بیاورید،
> بروکرها به دایرکتوری‌های خالی bind می‌شوند و journal تازه و **خالی** می‌سازند — یعنی از دست رفتن خاموش پیام‌ها.

<div dir="ltr">

```bash
# 1. dedicated mount, one directory per node
ROOT=/mnt/artemis            # a filesystem Docker does not share
sudo mkdir -p $ROOT/{live1,live2,live3,backup1,backup2,backup3}

# 2. stop every broker (backups first, then primaries) — NEVER `docker compose up` on this host
for c in artemis-backup1 artemis-backup2 artemis-backup3 \
         artemis-live1 artemis-live2 artemis-live3; do docker stop $c; done

# 3. copy each existing journal across, preserving ownership
for n in live1 live2 live3 backup1 backup2 backup3; do
  sudo cp -a /disk-c/docker/volumes/nova-dev_artemis-$n-data/_data/. $ROOT/$n/
done

# 4. point the stack at it and recreate, primaries first
echo "ARTEMIS_DATA_ROOT=$ROOT" >> .env

# 5. verify BEFORE trusting it: bindings must be non-empty and the pairs must re-form
for n in live1 live2 live3; do docker exec artemis-$n \
  ls -la /var/lib/artemis-instance/data/bindings; done
docker logs artemis-backup1 | grep AMQ221024      # synchronized with primary, nodeID=...
```

</div>

روی `AMQ222210` (هشدار آستانهٔ دیسک) و `AMQ212054` (بلاک شدن producer) alert بگذارید؛ رسیدن این دو یعنی مسیر
request/reply **همین حالا** متوقف است، نه اینکه به‌زودی متوقف می‌شود.
