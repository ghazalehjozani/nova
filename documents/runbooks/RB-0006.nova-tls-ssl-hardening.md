...
**هدف** — `REDIS_TLS_ENABLED=true` در env + CA در truststore. Nova از Lettuce با
`RedisClusterConfiguration` و `ClusterClientOptions` سفارشی (`RedisConfig`) استفاده می‌کند؛ گواهی CA همهٔ nodeهایی
که topology با `CLUSTER SHARDS` اعلام می‌کند باید در truststore JVM یا یک bundle جدا معتبر باشد. کلید YAML تغییری
لازم ندارد؛ فقط env var را `true` کنید و CA را در دسترس بگذارید.
``
...
graph LR
    ...
    Nova -->|"Redis TLS<br/>(فعلی REDIS_TLS_ENABLED=false)"| R[("Redis Cluster")]
...
- Redis Cluster: گواهی TLS باید SAN همهٔ primaryها و replicaهای اعلام‌شده را بپوشاند، چون Lettuce هنگام redirect،
  refresh توپولوژی و failover به endpoint کشف‌شده وصل می‌شود.
