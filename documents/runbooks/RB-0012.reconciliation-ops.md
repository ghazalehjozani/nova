...
# فهرست typeهای ثبت‌شده — در کاتالوگِ استانداردِ enum، بیرون از /ops
curl -sH "Accept-Language: fa" "$NOVA/v1/reference/enums/nova-reconciliation-type"
# {"name":"nova-reconciliation-type","entries":[{"code":"FACILITY_STATE","label":"وضعیت تسهیلات"}]}
```

</div>

> **هشدار — `code`ِ کاتالوگ را به API برنگردانید.** در `/v1/reference/enums` هر ورودی با **نامِ ثابتِ enum**
> کلید می‌خورد (`FACILITY_STATE`)، طبقِ قاعدهٔ آن کاتالوگ برای همهٔ enumها — نه با کدِ سیم (`facility-state`) که
> `reconciliationType.code` گزارش می‌کند و همهٔ ورودی‌ها آن را bind می‌کنند. فرستادنِ `FACILITY_STATE` به‌عنوان
> `reconciliationType` خطا نمی‌دهد؛ چون واژگان باز است، `200` با **صفر ردیف** برمی‌گردد. (ADR-0035)

> **شکلِ `reconciliationType` در پاسخ.** این فیلد در **پاسخ‌ها** یک شیء `{code,label}` است — مثل `status`،
> `verdict` و بقیهٔ فیلدهای واژگانی روی همان payload. مقدار قدیمیِ رشته‌ای دقیقاً همان `reconciliationType.code`
> است. در **ورودی‌ها** (query parameter، path variable، و متغیر `$RECON_TYPE` در دستورهای زیر) هیچ تغییری نکرده و
> همچنان کدِ خام پذیرفته می‌شود؛ ستون SQL `reconciliation_type` هم دست‌نخورده است. typeها یک واژگانِ باز هستند و
> Nova آن‌ها را ثبت می‌کند، پس یک typeِ بازنشسته همچنان قابلِ query است.
