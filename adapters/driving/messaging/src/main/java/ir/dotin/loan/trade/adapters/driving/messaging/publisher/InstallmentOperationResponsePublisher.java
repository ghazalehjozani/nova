...
/**
 * Publishes response messages to the corridor response topic as part of the request/reply pattern.
 *
 * <p>The response topic is extracted from the inbound message's {@code responseTopic} field. The {@code eventUid} is
 * used as the Kafka message key for correlation, and the {@code fileNumber} is set as a header for partition-aware
 * routing if needed.
 */
@Component
...
public class InstallmentOperationResponsePublisher {
    ...
    public void sendResponse(String responseTopic, InstallmentOperationResponse response) {
        ...
        try {
            ...
            LOG.error("Failed to serialize response [eventUid={}, topic={}]", response.eventUid(), responseTopic, e);
            return;
        }

        ProducerRecord<String, byte[]> record = new ProducerRecord<>(responseTopic, response.eventUid(), payload);

        record.headers()
                .add(new RecordHeader("eventUid", response.eventUid().getBytes(StandardCharsets.UTF_8)))
        ...
        future.whenComplete((result, ex) -> {
            if (ex != null) {
                LOG.error("Failed to publish response [eventUid={}, topic={}]", response.eventUid(), responseTopic, ex);
            } else {
                LOG.info(
                        "Response published [eventUid={}, topic={}, partition={}, offset={}]",
                        response.eventUid(),
                        responseTopic,
