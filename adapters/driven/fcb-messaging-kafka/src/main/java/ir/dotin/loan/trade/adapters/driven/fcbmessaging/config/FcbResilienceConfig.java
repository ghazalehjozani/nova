 ...
 *
 * <p>The previous {@code Resilience4j} circuit breaker was removed long ago, and the active heartbeat health gate that
 * briefly replaced it has now also been removed (the FCB health-check stack is gone). Outage handling is left to
 * bounded retries plus the per-call wall-time budget below.
 *
 * <p>Retry is now only applied to genuinely transient conditions and is intentionally bounded — in a financial system
 * you do not want to burn seconds retrying an already-broken downstream.
 *
 ...
 * </ol>
 *
 * <p>This is NOT a circuit breaker (forbidden here); it only bounds how long one call may spend retrying.
 */
@Configuration
