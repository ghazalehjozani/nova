...
import ir.dotin.loan.trade.adapters.driven.fcbmessaging.mapper.FcbTransactionMapper;
...
public class FcbTransactionAdapter implements TransactionPostingPort {
    ...
    public Result<TrackedTransactionNumber> postTransaction(LoanTransaction loanTransaction) {
        ...
        log.info(
                "Posting transaction via FCB corridor - facilityId: {}, trackingId: {}",
        ...
        Result<FcbBaseResponse> result = kafkaClient.sendAndReceive(request, transactionTimeout);

        if (result.isFailure()) {
            return Result.failure(result.err().orElseThrow());
        }
        FcbBaseResponse raw = result.unwrap();
        if (!(raw instanceof TransactionResultResponse response)) {
            return Result.failure(
                    Notification.ofError(CoreBankingErrors.FCB_INVALID_RESPONSE, "issue-general-document"));
        }

        return FcbTransactionMapper.mapToTrackedTransactionNumber(response, trackingId, clock);
    }
    ...
    public Result<List<TrackedTransactionNumber>> postTransactions(
        ...
        log.info(
                "Batch posting {} transactions via FCB corridor (merged into single document) for facility: {}",
    ...
    public Result<Unit> reverseTransaction(TrackedTransactionNumber transactionNumber) {
        log.debug("Reversing transaction via FCB corridor - {}", transactionNumber.value());
