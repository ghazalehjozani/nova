...
@Slf4j
@UtilityClass
public class FcbValidationMapper {

    // ── LoanServicePort mappings ──

    public Result<EconomicSector> mapToEconomicSector(EconomicSectorResponse response) {
        if (response.getCode() == null) {
            return Result.failure(
                    Notification.ofError(CoreBankingErrors.FCB_INVALID_RESPONSE, "loadEconomicalSectorByCode"));
        }
    ...
    public Result<EconomicalSectorValidation> mapToEcoSectorValidation(EcoSectorValidationResponse response) {
        if (response.getValid() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "validateEconomicalSectorForLoanType");
        }
        return Result.success(new EconomicalSectorValidation(response.getValid(), response.getMessage()));
    }

    public Result<ReasonType> mapToReasonType(ReasonTypeResponse response) {
        if (response.getCode() == null || response.getDescription() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "loadReasonType");
        }
    ...
    public Result<SubSource> mapToSubSource(ResourceResponse response) {
        if (response.getCode() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "loadResourceByCode");
        }
        return SubSource.of(response.getCode());
    }

    public Result<List<TopicInfo>> mapToTopicInfoList(TopicInfoListResponse response) {
        if (response.getTopics() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "loadTopicByCode");
        }
    ...
    public Result<BranchDetails> mapToBranchDetails(BranchDetailsResponse response) {
        if (response.getCode() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "loadBranch");
        }
    ...
    public Result<PartyInfoResponse> mapToPartyInfoResponse(
            CustomerInfoResponse response, PartyRole role, @Nullable BigDecimal guaranteePercentage) {
    ...
    public Result<PartyBirthInfo> mapToPartyBirthInfo(CustomerBirthInfoResponse response) {
    ...
    public Result<DepositInfo> mapToDepositInfo(DepositInfoResponse response) {
        if (response.getNumber() == null || response.getNumber().isBlank()) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "getDepositInfo");
        }
    ...
    public Result<DepositClosedStatus> mapToDepositClosedStatus(DepositClosedResponse response) {
        if (response.getClosed() == null || response.getCurrencyTypeCode() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "isDepositClosed");
        }
        return Result.success(new DepositClosedStatus(response.getClosed(), response.getCurrencyTypeCode()));
    }

    public Result<DebtorDepositValidation> mapToDebtorDepositValidation(ValidationResultResponse response) {
        if (response.getValid() == null) {
            return Result.failure(
                    Notification.ofError(CoreBankingErrors.FCB_INVALID_RESPONSE, "validateDebtorDeposit"));
        }
        return Result.success(new DebtorDepositValidation(response.getValid()));
    }

    public Result<CreditorDepositValidation> mapToCreditorDepositValidation(ValidationResultResponse response) {
        if (response.getValid() == null) {
            return Result.failure(
                    Notification.ofError(CoreBankingErrors.FCB_INVALID_RESPONSE, "validateCreditorDeposit"));
        }
        return Result.success(new CreditorDepositValidation(response.getValid()));
    }

    public Result<CurrencyValidation> mapToCurrencyValidation(CurrencyValidationResponse response) {
        if (response.getAllowed() == null) {
            return Result.failure(
                    Notification.ofError(CoreBankingErrors.FCB_INVALID_RESPONSE, "hasDepositAllowedCurrencies"));
        }
    ...
    public Result<CollateralValidation> mapToCollateralValidation(CollateralValidationResponse response) {
        if (response.getValid() == null) {
            return Result.failure(
                    Notification.ofError(CoreBankingErrors.FCB_INVALID_RESPONSE, "validateAddAssuranceToFile"));
        }
        return Result.success(new CollateralValidation(response.getValid(), response.getMessage()));
    }

    public Result<List<CollateralSerial>> mapToCollateralSerials(CollateralSerialsResponse response) {
        if (response.getSerials() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "reserveCollateral");
        }
    ...
    public Result<CollateralDetails> mapToCollateralDetails(CollateralDetailsResponse response) {
     ...
     * {@code FCB_INVALID_RESPONSE} failure instead.
     */
    ...
    public static Result<Unit> mapSamatViolationToNotification(ValidateSamatResponse validateSamatResponse) {
        if (validateSamatResponse == null || validateSamatResponse.getViolations() == null) {
            return Result.failure(CoreBankingErrors.FCB_INVALID_RESPONSE, "validateSamat");
        }

        List<SamatViolationDto> violations = validateSamatResponse.getViolations();
