...
public class AddGuarantorStep implements PublishingWriteActivity<AddGuarantorsCommandHandler.Data> {

    private final TradeLoanFacilityRepository facilityRepository;
    private final Clock clock;

    @Override
    public StepResult<List<DomainEvent<?>>> execute(WorkflowContext<AddGuarantorsCommandHandler.Data> ctx) {
        var data = ctx.data();

        var result = loadFacility(LoanFacilityId.of(data.facilityId())).flatMap(facility -> {
            Set<GuarantorParty> toAdd = Set.copyOf(data.guarantors());
            return facility.addGuarantors(toAdd, clock).map(ignored -> {
                List<DomainEvent<?>> events = List.copyOf(facility.domainEvents());
                facilityRepository.save(facility, data.expectedVersion());
                log.info("Added {} guarantor(s) for facility: {}", toAdd.size(), data.facilityId());
                return events;
            });
        });
