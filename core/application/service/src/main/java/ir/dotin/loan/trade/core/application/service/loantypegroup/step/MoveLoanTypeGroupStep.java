    ...
    public StepResult<List<DomainEvent<?>>> execute(WorkflowContext<MoveLoanTypeGroupData> ctx) {
        ...
        if (newParent != null) {
            ...
            if (repository.findAncestorChain(newParent).contains(id)) {
                return StepResult.fromWriteResult(Result.failure(LoanTypeGroupErrors.CYCLE_NOT_ALLOWED, id, newParent));
            }
        }
        ...
        Result<List<DomainEvent<?>>> result = repository
                ...
                .map(group -> group.moveUnder(newParent, clock)
                        .onSuccess(moved -> repository.save(moved, command.version()))
