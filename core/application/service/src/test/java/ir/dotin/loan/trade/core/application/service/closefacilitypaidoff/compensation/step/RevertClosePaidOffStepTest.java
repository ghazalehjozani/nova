...
class RevertClosePaidOffStepTest {
    ...
    @InjectMocks
    private RevertClosePaidOffStep step;
    ...
    private static WorkflowContextStub<CompensateCloseFacilityPaidOffCommandHandler.Data> ctx() {
        CompensateCloseFacilityPaidOffCommand command =
                new CompensateCloseFacilityPaidOffCommand(UUID.randomUUID(), 0L, "APP-1", "REF-1");
        return new WorkflowContextStub<>(
                "compensate-close-facility-paid-off",
                new CompensateCloseFacilityPaidOffCommandHandler.Data(command, Unit.INSTANCE));
    }
}
