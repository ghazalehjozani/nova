package ir.dotin.loan.trade.core.domain.disbursement.i18n;

import ir.dotin.platform.domain.common.i18n.LocalizedMessage;

public enum TradeDisbursementLocalizedMessageCodes implements LocalizedMessage<TradeDisbursementLocalizedMessageCodes> {
    INVALID_STATE_FOR_COMPLETION("Cannot complete Trade disbursement (ID: {0}) in state: {1}"),
    INVALID_STATE_FOR_PENDING("Cannot mark pending Trade disbursement (ID: {0}) in state: {1}"),
    INVALID_STATE_FOR_FAILURE("Cannot mark failed Trade disbursement (ID: {0}) in state: {1}"),
    TRANSACTION_NUMBERS_NULL_ON_COMPLETION(
            "Transaction Numbers cannot be null or empty for completed Trade disbursement (ID: {0})."),
    FAILURE_REASON_NULL("Failure reason cannot be null for failed Trade disbursement (ID: {0})."),
    ...
    TradeDisbursementLocalizedMessageCodes(String defaultMessageFormat) {
